  ...
  implements OnDestroyListener, Deleteable, MediaPlayer.OnCompletionListener, MediaPlayer.OnErrorListener, MediaPlayer.OnPreparedListener
{
    ...
    extends VideoView
  {
    ...
    private void onMeasure(final int paramInt1, final int paramInt2, final int paramInt3)
    {
      int m = 0;
      int j = 0;
      float f = VideoPlayer.this.container.$form().deviceDensity();
      Log.i("VideoPlayer..onMeasure", "Device Density = " + f);
      ...
      while (this.forcedWidth <= 64536)
      {
        j = VideoPlayer.this.container.$form().Width();
        if ((j == 0) && (paramInt3 < 2))
        {
          Log.d("VideoPlayer...onMeasure", "Width not stable... trying again (onMeasure " + paramInt3 + ")");
          VideoPlayer.this.androidUIHandler.postDelayed(new Runnable()
          {
            public void run()
            {
              VideoPlayer.ResizableVideoView.this.onMeasure(paramInt1, paramInt2, paramInt3 + 1);
            }
          }, 100L);
      ...
      for (;;)
      {
        if (this.forcedHeight <= 64536)
        {
          k = VideoPlayer.this.container.$form().Height();
          if ((k == 0) && (paramInt3 < 2))
          {
            Log.d("VideoPlayer...onMeasure", "Height not stable... trying again (onMeasure " + paramInt3 + ")");
            VideoPlayer.this.androidUIHandler.postDelayed(new Runnable()
            {
              public void run()
              {
          ...
          case -2147483648: 
            ...
            if (this.mFoundMediaPlayer.booleanValue())
            {
              try
              {
                i = this.mVideoPlayer.getVideoHeight();
                Log.i("VideoPlayer.onMeasure", "Got height from MediaPlayer>" + i);
                k = m;
              }
      ...
      for (;;)
      {
        Log.i("VideoPlayer.onMeasure", "Setting dimensions to:" + j + "x" + paramInt1);
        getHolder().setFixedSize(j, paramInt1);
    ...
    {
      onMeasure(paramInt1, paramInt2, 0);
    }
...
/* Location:              /home/nelmntrx/Desktop/SYNCCrowdTracker/dex2jar-2.0/classes-dex2jar.jar!/com/google/appinventor/components/runtime/VideoPlayer.class
